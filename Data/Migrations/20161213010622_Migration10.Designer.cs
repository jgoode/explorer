using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using explorer_api.Data;

namespace explorer_api.Data.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20161213010622_Migration10")]
    partial class Migration10
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
            modelBuilder
                .HasAnnotation("ProductVersion", "1.0.0-rtm-21431");

            modelBuilder.Entity("explorer_api.Models.ApplicationUser", b =>
                {
                    b.Property<string>("Id");

                    b.Property<int>("AccessFailedCount");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken();

                    b.Property<string>("Email")
                        .HasAnnotation("MaxLength", 256);

                    b.Property<bool>("EmailConfirmed");

                    b.Property<bool>("LockoutEnabled");

                    b.Property<DateTimeOffset?>("LockoutEnd");

                    b.Property<string>("NormalizedEmail")
                        .HasAnnotation("MaxLength", 256);

                    b.Property<string>("NormalizedUserName")
                        .HasAnnotation("MaxLength", 256);

                    b.Property<string>("PasswordHash");

                    b.Property<string>("PhoneNumber");

                    b.Property<bool>("PhoneNumberConfirmed");

                    b.Property<string>("SecurityStamp");

                    b.Property<bool>("TwoFactorEnabled");

                    b.Property<string>("UserName")
                        .HasAnnotation("MaxLength", 256);

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .HasName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasName("UserNameIndex");

                    b.ToTable("AspNetUsers");
                });

            modelBuilder.Entity("explorer_api.Models.Expedition", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<double>("BaseCredits");

                    b.Property<double>("BonusCredits");

                    b.Property<DateTime>("Created");

                    b.Property<double>("Distance");

                    b.Property<DateTime>("EndDate");

                    b.Property<int>("Jumps");

                    b.Property<DateTime>("StartDate");

                    b.Property<string>("Title");

                    b.Property<double>("TotalCredits");

                    b.Property<DateTime>("Updated");

                    b.Property<string>("UserId");

                    b.HasKey("Id");

                    b.ToTable("Expeditions");
                });

            modelBuilder.Entity("explorer_api.Models.Journal", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<DateTime>("Created");

                    b.Property<int>("JournalFileId");

                    b.Property<string>("JsonData");

                    b.Property<DateTime>("Updated");

                    b.HasKey("Id");

                    b.HasIndex("JournalFileId");

                    b.ToTable("Journals");
                });

            modelBuilder.Entity("explorer_api.Models.JournalFile", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<DateTime>("Created");

                    b.Property<string>("FileName")
                        .IsRequired();

                    b.Property<DateTime>("Updated");

                    b.Property<string>("UserId");

                    b.HasKey("Id");

                    b.HasAlternateKey("FileName")
                        .HasName("AlternateKey_FileName");

                    b.HasIndex("UserId");

                    b.ToTable("JournalFiles");
                });

            modelBuilder.Entity("explorer_api.Models.StarSystem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("Allegiance");

                    b.Property<DateTime>("Created");

                    b.Property<string>("Economy");

                    b.Property<int>("ExpeditionId");

                    b.Property<double>("FuelUsed");

                    b.Property<double>("FuleUsed");

                    b.Property<string>("Government");

                    b.Property<double>("JumpDistance");

                    b.Property<string>("Name");

                    b.Property<string>("Security");

                    b.Property<DateTime>("Updated");

                    b.Property<double>("X");

                    b.Property<double>("Y");

                    b.Property<double>("Z");

                    b.HasKey("Id");

                    b.HasIndex("ExpeditionId");

                    b.ToTable("StarSystems");
                });

            modelBuilder.Entity("explorer_api.Models.SystemObject", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<double>("AbsMagnitude");

                    b.Property<double>("AgeMy");

                    b.Property<string>("Atmosphere");

                    b.Property<DateTime>("Created");

                    b.Property<double>("Distance");

                    b.Property<double>("Eccentricity");

                    b.Property<bool>("Landable");

                    b.Property<string>("Name");

                    b.Property<double>("OrbitalPeriod");

                    b.Property<double>("OribitalInclination");

                    b.Property<double>("Periapsis");

                    b.Property<string>("PlanetClass");

                    b.Property<double>("Radius");

                    b.Property<double>("RotationalPeriod");

                    b.Property<double>("SemiMajorAxis");

                    b.Property<int>("StarSystemId");

                    b.Property<string>("StarType");

                    b.Property<double>("SurfaceGravity");

                    b.Property<double>("SurfacePressure");

                    b.Property<double>("SurfaceTemp");

                    b.Property<string>("TerraformState");

                    b.Property<int>("TidalLock");

                    b.Property<DateTime>("Updated");

                    b.Property<string>("Volcanism");

                    b.HasKey("Id");

                    b.HasIndex("StarSystemId");

                    b.ToTable("SystemObjects");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.EntityFrameworkCore.IdentityRole", b =>
                {
                    b.Property<string>("Id");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken();

                    b.Property<string>("Name")
                        .HasAnnotation("MaxLength", 256);

                    b.Property<string>("NormalizedName")
                        .HasAnnotation("MaxLength", 256);

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .HasName("RoleNameIndex");

                    b.ToTable("AspNetRoles");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.EntityFrameworkCore.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ClaimType");

                    b.Property<string>("ClaimValue");

                    b.Property<string>("RoleId")
                        .IsRequired();

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("AspNetRoleClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.EntityFrameworkCore.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd();

                    b.Property<string>("ClaimType");

                    b.Property<string>("ClaimValue");

                    b.Property<string>("UserId")
                        .IsRequired();

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserClaims");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.EntityFrameworkCore.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider");

                    b.Property<string>("ProviderKey");

                    b.Property<string>("ProviderDisplayName");

                    b.Property<string>("UserId")
                        .IsRequired();

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserLogins");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.EntityFrameworkCore.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId");

                    b.Property<string>("RoleId");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.HasIndex("UserId");

                    b.ToTable("AspNetUserRoles");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.EntityFrameworkCore.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId");

                    b.Property<string>("LoginProvider");

                    b.Property<string>("Name");

                    b.Property<string>("Value");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("AspNetUserTokens");
                });

            modelBuilder.Entity("explorer_api.Models.Journal", b =>
                {
                    b.HasOne("explorer_api.Models.JournalFile", "JournalFile")
                        .WithMany()
                        .HasForeignKey("JournalFileId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("explorer_api.Models.JournalFile", b =>
                {
                    b.HasOne("explorer_api.Models.ApplicationUser", "User")
                        .WithMany("JournalFiles")
                        .HasForeignKey("UserId");
                });

            modelBuilder.Entity("explorer_api.Models.StarSystem", b =>
                {
                    b.HasOne("explorer_api.Models.Expedition", "Expedition")
                        .WithMany("StarSystems")
                        .HasForeignKey("ExpeditionId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("explorer_api.Models.SystemObject", b =>
                {
                    b.HasOne("explorer_api.Models.StarSystem", "StarSystem")
                        .WithMany("SystemObjects")
                        .HasForeignKey("StarSystemId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.EntityFrameworkCore.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.EntityFrameworkCore.IdentityRole")
                        .WithMany("Claims")
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.EntityFrameworkCore.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("explorer_api.Models.ApplicationUser")
                        .WithMany("Claims")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.EntityFrameworkCore.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("explorer_api.Models.ApplicationUser")
                        .WithMany("Logins")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.EntityFrameworkCore.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.EntityFrameworkCore.IdentityRole")
                        .WithMany("Users")
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("explorer_api.Models.ApplicationUser")
                        .WithMany("Roles")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });
        }
    }
}
